%{?scl:%scl_package perl-Params-ValidationCompiler}

# Run optional test
%if ! (0%{?rhel}) && ! (0%{?scl:1})
%bcond_without perl_Params_ValidationCompiler_enables_optional_test
%bcond_without perl_Params_ValidationCompiler_enables_Class_XSAccessor
%else
%bcond_with perl_Params_ValidationCompiler_enables_optional_test
%bcond_with perl_Params_ValidationCompiler_enables_Class_XSAccessor
%endif

Name:		%{?scl_prefix}perl-Params-ValidationCompiler
Version:	0.30
Release:	6%{?dist}
...
%if %{with perl_Params_ValidationCompiler_enables_Class_XSAccessor}
BuildRequires:	%{?scl_prefix}perl(Class::XSAccessor)
%endif
BuildRequires:	%{?scl_prefix}perl(Eval::Closure)
BuildRequires:	%{?scl_prefix}perl(Exception::Class)
BuildRequires:	%{?scl_prefix}perl(Exporter)
BuildRequires:	%{?scl_prefix}perl(List::Util) >= 1.29
...
BuildRequires:	%{?scl_prefix}perl(Specio) >= 0.14
BuildRequires:	%{?scl_prefix}perl(Specio::Declare)
BuildRequires:	%{?scl_prefix}perl(Specio::Library::Builtins)
BuildRequires:	%{?scl_prefix}perl(Test2::Plugin::NoWarnings)
BuildRequires:	%{?scl_prefix}perl(Test2::Require::Module)
BuildRequires:	%{?scl_prefix}perl(Test2::V0)
BuildRequires:	%{?scl_prefix}perl(Test::More) >= 1.302015
BuildRequires:	%{?scl_prefix}perl(Test::Without::Module)
%if %{with perl_Params_ValidationCompiler_enables_optional_test}
# Optional Tests
BuildRequires:	%{?scl_prefix}perl(Const::Fast)
...
BuildRequires:	%{?scl_prefix}perl(Moose::Util::TypeConstraints)
BuildRequires:	%{?scl_prefix}perl(Types::Standard)
%endif
%endif
# Dependencies
...
%changelog
* Mon Jan 06 2020 Jitka Plesnikova <jplesnik@redhat.com> - 0.30-6
- SCL
